...
var CURRENT_STORY;
            ...
            else if (x.entryType.startsWith("boss_"))
                storyTypes.boss.push(x.id);
            else storyTypes.extra.push(x.id);
        });
        storyTypes.vn = []
            .concat(
                ...Object.values(vnStory.charEquip).map((x) => x.slice(1)),
            )
            .filter((x) => operatorData[vnStory.equipDict[x].charId]);
        storyTypes.vn.forEach(
            (x) =>
                (storyReview[x] = {
                    infoUnlockDatas: [
                        {
                            storyName: vnStory.equipDict[x].uniEquipName,
                            storyTxt: x,
                        },
                    ],
                }),
        );
        // this is available in lnlike_table, but were never translated (because they weren't used)
        storyTypes.ln.push("ln_0");
        storyReview["ln_0"] = {
        ...
        // for each lnlike:
        for (const [ln_key, ln_topic] of Object.entries(
            lnStory.topics,
        )) {
            storyTypes.ln.push(ln_key);
            storyReview[ln_key] = {
                name: ln_topic.name,
                infoUnlockDatas: [],
            };
            is_num = /_(\d)/.exec(ln_key)[1];
            is_name = `IS${parseInt(is_num) + 1}`;

            const endbook =
                lnStory.details[ln_key].archiveComp.endbook.endbook;
            if (!Object.keys(endbook).length) {
                // ro1 has no endbook, parse endings from "endings" instead:
                for (const [k, v] of Object.entries(
                    lnStory.details[ln_key].endings,
                )) {
                    ending_num = /_([^_]+)$/.exec(k)[1];
                    storyReview[ln_key].infoUnlockDatas.push({
                        storyName: v.name,
                        storyTxt: `obt/lnlike/ro${is_num}/level_ln${is_num}_ending_${ending_num}`,
                    });
                }
            } else {
                // for each endbook:
            ...
            Object.keys(lnStory.details[ln_key].monthSquad).forEach(
                (k, month_num) => {
                    let v = lnStory.details[ln_key].monthSquad[k];
                    let month_key = `${ln_key}_${k}`;
                    storyTypes.ln.push(month_key);
                    ...
                    if (
                        lnStory.details[ln_key].archiveComp.chat.chat[
                            v.chatId
                        ].clientChatItemData
                    ) {
                        for (const [k2, v2] of Object.entries(
                            ...
                            storyReview[month_key].infoUnlockDatas.push({
                                ...
                                storyBackground: `pic_${ln_key}_1`,
                            });
                        }
                    } else {
                        for (const [k2, v2] of Object.entries(
                ...
                case "vn":
                    namefunc = (n) => {
                        let name =
                            ...
                            operatorData[charCodeMap[n.split("_").slice(-1)[0]]]
                                .appellation;
                        let vnnum = parseInt(/uniequip_(\d+)/i.exec(n)[1]);
                        name += " [" + (vnnum - 1) + "]";
                        return name;
                    };
                    break;
...
async function genStory(data, avatars = []) {
    ...
    wordCount = 0;
    imgCount = 0;
    async function getvnStory(key) {
        return {
            //bg_corridor is a good alternate
            text: () =>
                `[background(image="bg_room_2")]\n[ShowItem(image="${
                    vnStory.equipDict[key].uniEquipIcon
                }",is_vn=1)]\n${escapeInvalidTags(
                    vnStory.equipDict[key].uniEquipDesc
                        .replace(/\n/g, "\\n")
                        .replace("[", "&#91;"),
                )}`,
            ok: true,
        ...
        .then((txt) => {
            ...
            function getWorkingScene(imageScene = false) {
                if (!scene) {
                    scene = createScene(
                        background("bg_black"),
                        background("bg_black", ASSET_SOURCE.ARKWAIFU),
                        imageScene,
                    );
                }
                return scene;
                ...
                if (
                    ...
                    switch (cmd.toLowerCase()) {
                        ...
                        case "image":
                            ...
                            let imgurl = background("bg_black");
                            let altimgurl = background(
                                "bg_black",
                            ...
                            switch (cmd.toLowerCase()) {
                                case "image":
                                    ...
                                    break;
                                case "background":
                                    imgurl = background(args.image);
                                    altimgurl = background(
                                        args.image,
